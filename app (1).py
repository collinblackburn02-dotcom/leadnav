...
def build_dashboard_views(orders_df, enriched_df, start_date, end_date, biz_type):
    ...
    if biz_type == "B2B / Enterprise Sales":
        summary_vars = [
            ...
            ("Region", "region"), ("State", "state_raw"), ("Income", "income"), ("Net Worth", "net_worth")
        ]
    else:
        summary_vars = [
            ("Gender", "gender"), ("Age", "age"), ("Marital Status", "marital_status"), 
            ("Region", "region"), ("State", "state_raw"), ("Zip Code", "zip_code"), 
            ("Income", "income"), ("Homeowner", "homeowner"), ("Children", "children"), ("Net Worth", "net_worth")
        ]
...
if st.session_state.app_state == "onboarding":
    ...
    with col1:
        ...
        st.session_state.orders_vault = st.file_uploader("Upload Shopify Order Export.", type=["csv"], accept_multiple_files=True, key="order_up")
    with col2:
